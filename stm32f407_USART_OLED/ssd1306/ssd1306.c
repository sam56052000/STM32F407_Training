...
extern void Delay_1us(uint32_t nCnt_1us);
extern void i2c_write(uint8_t address, uint8_t registry, uint8_t data);
...
void ssd1306_WriteCommand(uint8_t byte) {
    ...
    i2c_write(SSD1306_I2C_ADDR, 0x00, byte);
    HAL_Delay(10);
}

// Send data
void ssd1306_WriteData(uint8_t* buffer, uint8_t buff_size) {
	// HAL_I2C_Mem_Write(&SSD1306_I2C_PORT, SSD1306_I2C_ADDR, 0x40, 1, buffer, buff_size, HAL_MAX_DELAY);
    uint8_t i;

    for(i = 0; i < buff_size; i++)
    {
        i2c_write(SSD1306_I2C_ADDR, 0x40, buffer[i]);
        HAL_Delay(10);
    }
}
...
void ssd1306_Init(void) {
    ...
    ssd1306_WriteCommand(0x80); //-not offset
